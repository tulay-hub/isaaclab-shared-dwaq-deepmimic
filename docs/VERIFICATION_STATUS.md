...
- 10 个主项目 Public 仓库和第 11 个 `g1-motion-dataset` 文档仓库均已创建并推送到 `tulay-hub`；其中 G1 原始 CSV 没有上传，robot-retargeter 远程只保留框架/资产；
- 起身和共享仓库已改为精简版：只保留 MuJoCo 引用资产、最新 checkpoint/导出和必要数据，旧 checkpoint、历史日志、archive/deployment 没有进入远程；完整旧 staging 仍保存在本机 backup；
- 当前根 Git 仍无 commit、无 remote；远程上传使用了各独立 staging 仓库的 `main`，没有改写当前根仓库历史；
- `git-lfs` 已安装并用于各仓库的大文件；G1 数据仓库远程只发布文档，49G CSV 保存在本地备份，没有上传。
...
5. 使用 `git clone --recurse-submodules` 按各仓库 README 复现；需要完整 LFS 资产时执行 `git lfs pull`，并保留对应 commit、策略接口和回放证据。
