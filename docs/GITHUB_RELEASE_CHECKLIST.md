...
以下仓库已经在 `tulay-hub` 创建为 Public；训练权重、原始动作数据、body model、硬件 SDK 和未审计部署包仍按下面的规则排除或单独发布，不能因为仓库公开就直接提交。
...
| `g1-motion-dataset` | G1 动作组文档项目 | 当前只发布 README/许可证；原始 `bones_g1_origin` CSV 保留本地，不上传 |

独立仓库不能依赖当前工作区的根目录软链接。上传时应将 `framework/isaaclab_shared`、`data/raw/bvh` 等跨仓库入口转换为明确的 Git submodule 或 Release 下载说明；项目 02 的 PR/PR-UP/UL profile 留在项目 02，不把其 checkpoint 和踝映射伪装成共享默认。`robot-retargeter-smplx` 的 `bones_g1_origin` 原始动作组单独保留在本地，当前不进入任何 Public 仓库。
...
- 不执行 `git push`，除非用户明确要求并确认目标远程/分支；本次 Public 上传已得到用户明确要求，目标为 `tulay-hub` 的 `main`。
