...
class PPO:
    ...
    def __init__(
        ...
        # PPO components
        self.policy = policy
        # fixed_action_std: 探索 std 固定不参与训练 (防止 entropy 项把 std 推爆)
        self.fixed_action_std = fixed_action_std
        if self.fixed_action_std and hasattr(self.policy, "std"):
            self.policy.std.requires_grad_(False)
        self.action_mean_l2_coef = action_mean_l2_coef
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            # 动作均值 L2 惩罚: 防止 actor 输出均值飘到 ±40 (退化策略), 逼均值回到 0 附近
            if self.action_mean_l2_coef > 0:
                loss = loss + self.action_mean_l2_coef * mu_batch.square().mean()
