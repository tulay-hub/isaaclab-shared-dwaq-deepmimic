"""不启动 Isaac, 直接把 Lens110 159 维 DeepMimic checkpoint 转成 ONNX + deploy_config。"""
...
STIFF = [40, 40, 40, 40, 55, 55, 40, 40, 40, 40, 55, 55, 100, 20, 20, 20, 20, 20, 20, 20, 20]
DAMP = [5] * 21
OBS_NAMES = [
...
def main():
    ...
    export_dir = os.path.join(os.path.dirname(os.path.abspath(ckpt)), "exported_159")
    ...
    torch.onnx.export(
        ...
        os.path.join(export_dir, "policy_159.onnx"),
        export_params=True,
        opset_version=11,
        input_names=["obs"],
    ...
    with open(os.path.join(export_dir, "deploy_config_159.yaml"), "w", encoding="utf-8") as f:
        yaml.dump(deploy, f, allow_unicode=True, default_flow_style=None)
    print("exported:", os.path.join(export_dir, "policy_159.onnx"))
