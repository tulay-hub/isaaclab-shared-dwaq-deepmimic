...
parser.add_argument(
    ...
    type=str,
    default="atom02", 
    choices=["g1", "atom01", "atom01_long_base_link", "atom02", "a1", "lens110"],
...
parser.add_argument(
    "--skip-key-body-sim",
    action="store_true",
    help="Skip Isaac replay and save only remapped root/joint motion data.",
)
...
elif args_cli.robot == "lens110":
    repo_root = Path(__file__).resolve().parents[3]
    lens110_cfg_path = repo_root / "source/legged_lab/legged_lab/data/Robots/model_humanoid_lens110/lens110.py"
    spec = importlib.util.spec_from_file_location("lens110_cfg", lens110_cfg_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load Lens110 config from {lens110_cfg_path}")
    lens110_cfg = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = lens110_cfg
    spec.loader.exec_module(lens110_cfg)
    ROBOT_CFG = lens110_cfg.LENS110_CFG
...
def main():
    ...
    if args_cli.skip_key_body_sim:
        print("Skipping Isaac replay. Outputs will not contain key_body_pos.")
    else:
