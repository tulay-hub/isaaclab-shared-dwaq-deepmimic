...
are converted to upper/lower motor targets by solving the XML spatial tendon
geometry.
"""
...
SOLVE_DIR = REPO_ROOT_HINT / "pitchRoll2UpperLower" / "solve"
...
DEFAULT_OUTPUT = Path(__file__).resolve().parents[2] / "outputs" / "lens110_pr_xml_tendon.mp4"
UPPER_LOWER = (
...
class XmlTendonPrToUpperLower:
    def __init__(self, xml_path: Path, default_pr: np.ndarray):
        self.solver = Lens110XmlTendonSolver(xml_path)
        self.last_ul: np.ndarray | None = None
    ...
    def __call__(self, pr: np.ndarray) -> np.ndarray:
        ...
        ul = self.solver.solve_pr_to_ul(pr[0], pr[1], pr[2], pr[3], initial=self.last_ul)
...
def target_pr_to_control(
    ...
    cfg: dict,
    control_default: np.ndarray,
    mapper: XmlTendonPrToUpperLower,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Lens110 PR policy on upper/lower ankle MJCF with XML tendon mapping.")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG))
    parser.add_argument("--policy", default=None)
    parser.add_argument("--xml", default=None, help="Override upper/lower MuJoCo XML path.")
    ...
    parser.add_argument("--no_render", action="store_true", help="Run physics without viewer or video writer.")
...
def main() -> None:
    ...
    print("[INFO] PR->UL: XML spatial tendon solve, no pkl/MLP/polynomial model")
    print(
    ...
    if args.headless and writer is not None:
        writer.release()
