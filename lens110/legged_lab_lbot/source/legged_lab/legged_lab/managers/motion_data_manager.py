...
class MotionDataTerm(ManagerTermBase):
    ...
    def _load_motion_data(self):
        ...
        self.manual_avg_vels = []  # Average body-frame velocities used for command-conditioned sampling.
        ...
        for motion_name, value in self.motion_weights_dict.items():
            # Parse value: tuple(weight, avg_vel), tuple(weight, "auto"), or a bare weight.
            if isinstance(value, (list, tuple)) and len(value) == 2:
                motion_weight, avg_vel_spec = value
            elif isinstance(value, (int, float)):
                motion_weight = float(value)
                avg_vel_spec = "auto"
            else:
                raise ValueError(
                    f"Motion '{motion_name}' must be weight or tuple(weight, [vx, vy, wz]/'auto'), got {value!r}."
                )

            if isinstance(avg_vel_spec, str) and avg_vel_spec.lower() != "auto":
                raise ValueError(f"Motion '{motion_name}' has unknown velocity spec '{avg_vel_spec}'.")

            if isinstance(avg_vel_spec, (list, tuple)):
                if len(avg_vel_spec) != 3:
                    raise ValueError(f"Motion '{motion_name}' avg velocity must have 3 values, got {avg_vel_spec!r}.")
                # Convert to tensor, assume [vx, vy, wz]
                avg_vel_tensor = torch.tensor(avg_vel_spec, dtype=torch.float32, device=self.device)
            else:
                avg_vel_tensor = None
            ...
            if avg_vel_tensor is None:
                ...
                avg_vel_tensor = torch.stack(
                    [
                        root_vel_b[:, 0].mean(),
                        root_vel_b[:, 1].mean(),
                        root_ang_vel_b[:, 2].mean(),
                    ]
                )
                avg_vel_tensor = torch.where(avg_vel_tensor.abs() < 0.03, torch.zeros_like(avg_vel_tensor), avg_vel_tensor)
            avg_vel_tensor.requires_grad_(False)
