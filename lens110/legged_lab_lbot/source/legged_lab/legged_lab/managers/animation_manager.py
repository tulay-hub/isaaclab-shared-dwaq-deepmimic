...
class AnimationTerm(ManagerTermBase):
    ...
    def _get_base_velocity_commands(self) -> torch.Tensor:
        """Get the base-velocity commands used to condition motion sampling.

        Returns the command from the command manager if the environment defines one (e.g. AMP).
        For tasks that do not define a ``base_velocity`` command (e.g. DeepMimic reference
        tracking), returns zero commands so that motion sampling relies only on the current
        velocity via :attr:`cfg.velocity_blend_ratio`.
        """
        if "base_velocity" in self._env.command_manager.active_terms:
            return self._env.command_manager.get_command("base_velocity")
        return torch.zeros(self.num_envs, 3, device=self._env.device)
    ...
    def reset(self, env_ids: Sequence[int] | None = None):
        ...
        commands = self._get_base_velocity_commands()
