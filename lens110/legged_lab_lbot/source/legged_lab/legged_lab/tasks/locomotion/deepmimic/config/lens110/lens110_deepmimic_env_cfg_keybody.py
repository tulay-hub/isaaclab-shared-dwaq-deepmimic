...
class Lens110DeepMimicEnvCfg(DeepMimicEnvCfg):
    """Lens110 舞蹈任务（F 版备份：含关键点观测，251 维，训练效果最优）。

    消融实验结论：删掉关键点观测（key_body_pos_b + ref_key_body_pos_b）后
    总奖励仅降 ~5%（24.2 → 23.0），但关键点偏移终止率 +70%（3.3% → 5.6%）。
    本文件保留 F 版供备份；真机部署用 H 版（无关键点观测，161 维）见
    ``lens110_deepmimic_env_cfg.py``。
    """

    def __post_init__(self):
        super().__post_init__()

        # The dance clip is 1812 frames @ 50 Hz = 36.22 s; give a small margin.
        ...
        # The "_reorder" pkl is in Isaac Lab dof order (interleaved left/right pairs),
        # matching the robot's articulation dof order. Do NOT use the non-reorder pkls.
        self.motion_data.motion_dataset.motion_data_weights = {
            "lens110_tangbohushuo_50hz_reorder": 1.0,
        }

        # -----------------------------------------------------
        # Actions
        # -----------------------------------------------------
        # 动作缩放：基础配置默认 1.0，改为 0.25（与 AMP 任务一致）
        ...
        # F 版：保留关键点观测（key_body_pos_b 18 维 + ref_key_body_pos_b 72 维）→ 251 维
