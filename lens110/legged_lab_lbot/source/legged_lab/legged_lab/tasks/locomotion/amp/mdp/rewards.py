...
def lens110_arm_symmetry_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Penalize left-right arm pose mismatch for Lens110.

    Pitch and elbow joints should match; roll and yaw joints should mirror signs.
    """
    asset: Articulation = env.scene[asset_cfg.name]
    joint_pos = asset.data.joint_pos
    pitch = joint_pos[:, 13] - joint_pos[:, 17]
    roll = joint_pos[:, 14] + joint_pos[:, 18]
    yaw = joint_pos[:, 15] + joint_pos[:, 19]
    elbow = joint_pos[:, 16] - joint_pos[:, 20]
    return pitch.square() + roll.square() + yaw.square() + elbow.square()


def lens110_arm_vel_symmetry_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Penalize one-sided arm swing speed for Lens110."""
    asset: Articulation = env.scene[asset_cfg.name]
    joint_vel = asset.data.joint_vel
    pitch = joint_vel[:, 13] - joint_vel[:, 17]
    roll = joint_vel[:, 14] + joint_vel[:, 18]
    yaw = joint_vel[:, 15] + joint_vel[:, 19]
    elbow = joint_vel[:, 16] - joint_vel[:, 20]
    return pitch.square() + roll.square() + yaw.square() + elbow.square()


def lens110_mjcf_arm_symmetry_l2(
    env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Penalize Lens110 MJCF arm mismatch using resolved joint-name order."""
    asset: Articulation = env.scene[asset_cfg.name]
    joint_pos = asset.data.joint_pos[:, asset_cfg.joint_ids]
    pitch = joint_pos[:, 0] - joint_pos[:, 4]
    roll = joint_pos[:, 1] + joint_pos[:, 5]
    yaw = joint_pos[:, 2] + joint_pos[:, 6]
    elbow = joint_pos[:, 3] + joint_pos[:, 7]
    return pitch.square() + roll.square() + yaw.square() + elbow.square()


def lens110_mjcf_arm_vel_symmetry_l2(
    env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Penalize Lens110 MJCF arm velocity mismatch using resolved joint-name order."""
    asset: Articulation = env.scene[asset_cfg.name]
    joint_vel = asset.data.joint_vel[:, asset_cfg.joint_ids]
    pitch = joint_vel[:, 0] - joint_vel[:, 4]
    roll = joint_vel[:, 1] + joint_vel[:, 5]
    yaw = joint_vel[:, 2] + joint_vel[:, 6]
    elbow = joint_vel[:, 3] + joint_vel[:, 7]
    return pitch.square() + roll.square() + yaw.square() + elbow.square()


def lens110_hip_yaw_symmetry_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Penalize asymmetric hip yaw and excessive mirrored yaw spread."""
    asset: Articulation = env.scene[asset_cfg.name]
    joint_pos = asset.data.joint_pos
    default_joint_pos = asset.data.default_joint_pos
    symmetry_error = joint_pos[:, 2] + joint_pos[:, 8]
    default_error = (joint_pos[:, 2] - default_joint_pos[:, 2]).square() + (
        joint_pos[:, 8] - default_joint_pos[:, 8]
    ).square()
    return symmetry_error.square() + 0.5 * default_error


def lens110_mjcf_hip_yaw_symmetry_l2(
    env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Penalize Lens110 MJCF hip yaw asymmetry using resolved joint-name order."""
    asset: Articulation = env.scene[asset_cfg.name]
    joint_pos = asset.data.joint_pos[:, asset_cfg.joint_ids]
    default_joint_pos = asset.data.default_joint_pos[:, asset_cfg.joint_ids]
    symmetry_error = joint_pos[:, 0] + joint_pos[:, 1]
    default_error = (joint_pos[:, 0] - default_joint_pos[:, 0]).square() + (
        joint_pos[:, 1] - default_joint_pos[:, 1]
    ).square()
    return symmetry_error.square() + 0.5 * default_error
...
def base_height(
    ...
    # Penalize both below-target and above-target deviations.
