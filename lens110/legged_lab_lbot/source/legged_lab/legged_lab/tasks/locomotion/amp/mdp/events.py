...
def pin_joints_to_default_target(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
):
    """Pin the position drive targets of the selected joints to their default pose.

    Joints that are not covered by any action term keep their PhysX drive targets at
    0.0, so the PD controllers yank them away from the default pose at every reset
    (e.g. the arms snap from the bent default pose to the zero pose). This event
    re-pins the drive targets so those joints are held at the default pose instead.

    The targets persist for the whole episode because only the action manager
    overwrites ``joint_pos_target`` for the joints it covers.
    """
