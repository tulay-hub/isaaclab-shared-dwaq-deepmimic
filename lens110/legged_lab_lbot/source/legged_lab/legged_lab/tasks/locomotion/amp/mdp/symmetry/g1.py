...
def _transform_policy_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    negating certain components of the angular velocities, projected gravity,
    velocity commands, and flipping the joint positions, joint velocities, and last
    actions for the G1 29-DOF robot.

    Note:
        The G1 AMP policy observation layout (96 dims, no history) is:
    ...
    end_idx = 0
    # base ang vel: negate roll (x) and yaw (z)
    start_idx = end_idx
    ...
    start_idx = end_idx
    end_idx = start_idx + PROJ_GRAVITY_DIM
    obs[:, start_idx:end_idx] = obs[:, start_idx:end_idx] * torch.tensor([1, -1, 1], device=device)
    # velocity command: negate lin_vel_y and ang_vel_z
    start_idx = end_idx
