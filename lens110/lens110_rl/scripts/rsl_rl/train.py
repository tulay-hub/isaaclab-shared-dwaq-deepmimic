...
def _dump_pickle(filename: str, data: object) -> None:
    """Pickle 配置到 log 目录 (Isaac Lab 新版移除了 dump_pickle)。"""
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.motion_file is not None:
        ...
        # 回退到任务配置里已经写好的 motion 文件 (本地训练更省事)
        env_cfg.commands.motion.motion_file = os.path.abspath(env_cfg.commands.motion.motion_file)
        print(f"[INFO] Using motion file from task config: {env_cfg.commands.motion.motion_file}")
    ...
    if agent_cfg.resume:
        # 支持直接传 checkpoint 文件绝对路径 (不依赖 run 目录)
